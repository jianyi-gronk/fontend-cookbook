...
describe("MultiHook", () => {
	...
	it("test", () => {
		const hook1 = SyncHook(["a", "b"]);
		const hook2 = expect(new MultiHook([hook1, hook1]).isUsed()).toBe(true);
		expect(new MultiHook([hook1, fakeHook2]).isUsed()).toBe(true);
		expect(new MultiHook([fakeHook2, hook1]).isUsed()).toBe(true);
		expect(new MultiHook([fakeHook2, fakeHook2]).isUsed()).toBe(false);
	});
});
